...
credentials.users.forEach((user, index) => {
    ...
    test(`Open dispute details - User: ${username} (${index + 1}/${credentials.users.length})`, async ({ page }) => {
        ...
        await test.step('Open Dispute detail', async () => {
            ...
            await page.locator('a', { hasText: 'Dispute details' }).click();
            const committedContent = page.locator('#dispute-content-wrap');
            await expect(committedContent).toBeVisible({ timeout: 60_000 });
            console.log('Dispute details opened is Visible');
