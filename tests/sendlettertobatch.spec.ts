...
credentials.users.forEach((user, index) => {
    ...
    test(`Send en-masse letter to batch from Action list screen- User: ${username} (${index + 1}/${credentials.users.length})`, async ({ page }) => {
        ...
        await test.step('Open List screens-Debtors, Invoices, Disputes, Requests, Customer groups and Actions including their submenus', async () => {
            ...
            const actionLink = page.locator('a').filter({ hasText: /^Actions$/ });
            ...
            await page.locator('input[name="pageSize"]').fill('20');
            await page.locator('input[name="pageSize"]').press('Enter');
            await expect(page.locator('.window-title-text')).toHaveText('Letter actions');
            await page.locator('.ag-center-cols-container .ag-row').first().waitFor({ state: 'visible', timeout: 120000 });
            await page.getByLabel('Press Space to toggle all').check({ timeout: 90000 });
            await page.locator('a').filter({ hasText: 'Send to CreditNext Batch' }).click();
            await expect(page.locator('div').filter({ hasText: /^20 letter\(s\) and \/ or email\(s\) sent to batch, 0 other action type\(s\) skipped$/ })).toBeVisible({ timeout: 30_000 });
