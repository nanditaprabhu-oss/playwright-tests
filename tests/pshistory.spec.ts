...
credentials.users.forEach((user, index) => {
    ...
    test(`Open Payment scheme details - User: ${username} (${index + 1}/${credentials.users.length})`, async ({ page }) => {
        ...
        await test.step('Open PS detail', async () => {
            ...
            await expect(page.getByText('Open', { exact: true })).toBeVisible({ timeout: 60000 });
            await page.locator('a', { hasText: 'Payment scheme details' }).click();
            const committedContent = page.locator('#payment-scheme-content-wrap');
            await expect(committedContent).toBeVisible({ timeout: 60_000 });
            console.log('Payment Scheme details is Visible');
