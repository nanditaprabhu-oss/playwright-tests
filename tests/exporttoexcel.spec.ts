...
credentials.users.forEach((user, index) => {
  ...
  test(`Export to excel - User: ${username} (${index + 1}/${credentials.users.length})`, async ({ page }) => {
    ...
    // Open debtor list screen>>Go to Debtors with outstanding invoices drop down menu
      ...
      await page.locator('label[for="sizeDimensions1"]').click();
      ...
      await page.locator('.ag-center-cols-container .ag-row').first().waitFor({state: 'visible',timeout: 25000});
    ...
    await test.step('Export Dispute excel', async () => {
      ...
      // Add timeout to waitForEvent because the exports can take a little longer than the default timeout
      const download3Promise = page.waitForEvent('download', { timeout: 250000 });
