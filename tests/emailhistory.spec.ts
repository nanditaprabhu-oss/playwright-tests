...
credentials.users.forEach((user, index) => {
    ...
    test(`Open Email history- User: ${username} (${index + 1}/${credentials.users.length})`, async ({ page }) => {
        ...
        await test.step('Open Email detail', async () => {
            ...
            const statementCell = page.getByRole('gridcell', { name: 'E-mail action' }).first();
            await expect(statementCell).toBeVisible({ timeout: 90000 });
            await statementCell.dblclick();
            await expect(page.locator('.dxrePage')).toBeVisible({ timeout: 90000 });
            console.log('Email details is Visible');
