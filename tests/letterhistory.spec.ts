...
credentials.users.forEach((user, index) => {
    ...
    test(`Open letter details from letter history screen- User: ${username} (${index + 1}/${credentials.users.length})`, async ({ page }) => {
        ...
        await test.step('Open Letter detail', async () => {
            ...
            await page.waitForTimeout(3000); // Wait after checkbox
            await page.locator('a').filter({ hasText: 'History dunning letters' }).click({ timeout: 90000 });
            const statementCell = page.getByRole('gridcell', { name: 'Statement' }).first();
            await expect(statementCell).toBeVisible({ timeout: 90000 });
            await statementCell.click();
            //await page.locator('.ag-cell', { hasText: 'Statement' }).click({ timeout: 90000 });
            await page.locator('a').filter({ hasText: /^Letter$/ }).click();
            await expect(page.locator('.dxrePage')).toBeVisible({ timeout: 90000 });
            console.log('Letter details is Visible');
